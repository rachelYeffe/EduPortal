...
import { ProfilePopupServiceService } from '../../Services/profile-popup-service.service';
...
export class ProfilePopupComponent {
  ...
  constructor(private profilePopupServiceService:ProfilePopupServiceService ) { }
  getYeshivaStudent(idStudent: string) {
    this.profilePopupServiceService.GetYeshivaStudentById(idStudent).subscribe(res => {
      const s = res;
    ...
    getGraduate(idGraduate: string) {
    this.profilePopupServiceService.GetGraduateById(idGraduate).subscribe(res => {
      const g = res;
      const green = 'style="color: #4a90e2;"';
